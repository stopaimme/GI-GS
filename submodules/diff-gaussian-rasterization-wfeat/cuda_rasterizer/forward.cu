...
// Maximum feature channels supported via local-memory accumulator.
// Compile-time specialised kernels keep this in registers for small dims.
	...
	bool inference)
{
	...
	// Feature accumulator: for feat_channels <= ~32 this stays in registers;
	// for larger dims the compiler places it in L1-cached local memory.
	float F[MAX_FEAT_CHANNELS];
	for (int ch = 0; ch < feat_channels; ch++) F[ch] = 0.0f;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Arbitrary-dim feature accumulation with float4 vectorised loads.
			// float4 path handles 4 channels per iteration; scalar tail handles remainder.
			if (gs_feature != nullptr) {
				const float* feat_j = gs_feature + collected_id[j] * feat_channels;
				int ch = 0;
				for (; ch <= feat_channels - 4; ch += 4) {
					const float4 v = reinterpret_cast<const float4*>(feat_j + ch)[0];
					F[ch+0] += v.x * weight;
					F[ch+1] += v.y * weight;
					F[ch+2] += v.z * weight;
					F[ch+3] += v.w * weight;
	...
	if (inside)
	{
		...
		// Write arbitrary-dim feature output: layout [feat_channels, H, W]
