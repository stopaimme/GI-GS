...
#ifndef MAX_FEAT_CHANNELS
#define MAX_FEAT_CHANNELS 512
#endif
...
renderCUDA(
	...
	const float* __restrict__ gs_feature,
	const int feat_channels,
	...
	float* __restrict__ dL_dfeature)
{
	...
	// Per-pixel upstream gradient for the arbitrary-dim feature.
	// Layout of dL_dpixels_feature is [feat_channels, H, W] — strided per channel.
	float dL_dpixel_feat[MAX_FEAT_CHANNELS];
	if (inside) {
		for (int i = 0; i < C; i++) {
		...
		if (dL_dpixels_feature != nullptr) {
			for (int ch = 0; ch < feat_channels; ch++) {
				dL_dpixel_feat[ch] = dL_dpixels_feature[ch * H * W + pix_id];
			}
		} else {
			for (int ch = 0; ch < feat_channels; ch++) dL_dpixel_feat[ch] = 0.0f;
		}
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Arbitrary-dim feature gradient: dL/d(feature_i[ch]) = dchannel_dcolor * dL/d(out_feature[ch])
			// float4 writes where possible to reduce atomicAdd call overhead.
			if (dL_dfeature != nullptr) {
				float* grad_feat_j = dL_dfeature + global_id * feat_channels;
				int ch = 0;
				for (; ch <= feat_channels - 4; ch += 4) {
					atomicAdd(grad_feat_j + ch+0, dchannel_dcolor * dL_dpixel_feat[ch+0]);
					atomicAdd(grad_feat_j + ch+1, dchannel_dcolor * dL_dpixel_feat[ch+1]);
					atomicAdd(grad_feat_j + ch+2, dchannel_dcolor * dL_dpixel_feat[ch+2]);
					atomicAdd(grad_feat_j + ch+3, dchannel_dcolor * dL_dpixel_feat[ch+3]);
				}
				for (; ch < feat_channels; ch++) {
					atomicAdd(grad_feat_j + ch, dchannel_dcolor * dL_dpixel_feat[ch]);
				}
			}
