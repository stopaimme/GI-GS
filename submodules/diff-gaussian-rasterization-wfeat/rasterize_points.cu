...
RasterizeGaussiansCUDA(
    ...
    const torch::Tensor& gs_feature,  	// [P, feat_channels] — arbitrary dim
	...
	const int H = image_height;
	const int W = image_width;
	// Derive feat_channels at runtime from the input tensor shape.
	// gs_feature may be an empty tensor (size 0) when feature is not used.
	const bool has_feature = (gs_feature.numel() > 0);
	const int feat_channels = has_feature ? (int)gs_feature.size(1) : 0;
	...
	// out_feature layout: [feat_channels, H, W]. Empty when feat_channels == 0.
	torch::Tensor out_feature = has_feature
		? torch::full({feat_channels, H, W}, 0.0, float_opts)
	...
	if(P != 0) {
		...
		rendered = CudaRasterizer::Rasterizer::forward(
			...
			has_feature ? gs_feature.contiguous().data<float>() : nullptr,
			feat_channels,
			scales.contiguous().data_ptr<float>(),
			scale_modifier,
			rotations.contiguous().data_ptr<float>(),
			...
			has_feature ? out_feature.contiguous().data<float>() : nullptr,
			radii.contiguous().data<int>(),
			debug);
  	}
  	return std::make_tuple(
...
RasterizeGaussiansBackwardCUDA(
    ...
    const torch::Tensor& dL_dout_feature,  // [feat_channels, H, W]
	...
	// Per-Gaussian feature gradient: layout [P, feat_channels]
	torch::Tensor dL_dfeature = has_feature
		? torch::zeros({P, feat_channels}, means3D.options())
		: torch::empty({0}, means3D.options());
	...
	if(P != 0) {
		CudaRasterizer::Rasterizer::backward(P, degree, M, R,
			...
			(has_feature && dL_dout_feature.numel() > 0)
				? dL_dout_feature.contiguous().data<float>() : nullptr,
			...
			has_feature ? dL_dfeature.contiguous().data<float>() : nullptr,
